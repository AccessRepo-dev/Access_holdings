...
with source as (
    SELECT
        -- Identifiers
        CAST(e.RECORDNO AS TEXT) AS TRANSACTIONS_UNIQUE_ID,
        CAST(e.BATCHNO AS NUMBER) AS TRANSACTION_ID,
        e.RECORDNO AS TRANSACTION_LINE_ID,
        e.BATCHNO AS TRANSACTION_NUMBER,
        e.BATCHTITLE AS TRANID,
        CAST(e.TR_TYPE AS TEXT) AS TRANSACTION_TYPE,
        e.STATE AS STATUS,
        ...
        TRUE AS IS_POSTING,
        CAST(COALESCE(acc.ACCOUNTNO, E.ACCOUNTNO) AS TEXT) AS ACCOUNT_NUMBER,
